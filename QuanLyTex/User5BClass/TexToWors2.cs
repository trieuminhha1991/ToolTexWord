...
namespace QuanLyTex.User5Class
{
	class TexToWord2
	{
		...
		public string startTex(string tex)
		{
			try
			{
				tex = tex.Replace(@"\motcot", @"\choice");
				tex = tex.Replace(@"{,}", ",");
				tex = tex.Replace("’", "'").Replace(@"\noindent", "");
				tex = tex.Replace(@"\newline ", "\\");
				tex = tex.Replace(@"\hfill", "");
				tex = tex.Replace(@"\tag", "");
				tex = tex.Replace(@"\wideparen", @"\overset\frown");
				tex = tex.Replace(@"\[", "$").Replace(@"\]", "$").Replace("{'}", "'");
				tex = tex.Replace(@"alignat*", "align").Replace(@"alignat", "align");
				tex = tex.Replace(@"\begin{align}", "\n\\begin{align}").Replace(@"\begin{align*}", "\n\\begin{align}").Replace(@"\begin{eqnarray}", "\n\\begin{align}").Replace(@"\begin{eqnarray*}", "\n\\begin{align}").Replace(@"\begin{equation}", "\n\\begin{align}").Replace(@"\begin{equation*}", "\n\\begin{align}").Replace(@"\begin{array}", "\n\\begin{align}");
				tex = tex.Replace(@"\end{align}", "\\end{align}\n").Replace(@"\end{align*}", "\\end{align}\n").Replace(@"\end{eqnarray*}", "\\end{align}\n").Replace(@"\end{equation}", "\\end{align}\n").Replace(@"\end{equation*}", "\\end{align}\n").Replace(@"\end{array}", "\\end{align}\n");
				tex = tex.Replace(@"\immini", "");
				tex = tex.Replace(@"\lbrace", "{").Replace(@"\rbrace", "}");
				tex = tex.Replace(@"\left{", @"\left\{").Replace(@"\right}", @"\right\}");
				Regex rx = new Regex(@"\$([ ]{0,2})([A-Za-z0-9'., ]{1,12})([ ]{0,2})\$");
				if (rx.IsMatch(tex))
				{
				...
				while (tex.Contains("$$"))
				{
					...
					if (tex.Substring(start + 2, 5).Contains("\n"))
					{
						tex = tex.Remove(start, 2).Insert(start, "$\n");
					}
					else
					{
						tex = tex.Remove(start, 2).Insert(start, "$ $");
		...
		public string runLatexImage(string tiz, bool? fixImage)
		{
			try
			{
				if (fixImage == true)
				{
					string header = @"\documentclass[tikz,border = 2mm]{standalone}
                                                \usepackage{amsmath}
                                                \usepackage{amsfonts}
                                                \usepackage{amssymb}
		...
		private string DeleteName(string tex)
		{
			if (deleteName == true)
			{
				string check = stringName;
		...
		private List<string> getIdQuestion(List<string> list)
		{
			try
			{
		...
		public Dictionary<string, dynamic> changeTexToWord(string tex, bool? fixImage)
		{
			imageAdd = "";
			try
			{
		...
		public void addTextToWord(List<string> list, string path, bool? ToogleTex, bool? fixImage, bool? selectFilter, bool? deleteCheck, string NameDuAn, bool? addTable, bool? addPdf, bool? Runword, Application app)
		{
			...
			try
			{
				...
				level2.StartAt = 1;
				level2.Font.Bold = 1;
				level2.Font.Color = WdColor.wdColorBlue;
				...
				foreach (string item in list)
				{
					...
					{
						string chtrue = "";
						Dictionary<string, dynamic> Dic = changeTexToWord(item, fixImage);
				...
				if (selectFilter == true)
				{
					try
					{
						...
						while (find.Execute())
						{
							...
							else if (rangeend.Text.Contains("dl"))
							{
								rangeend.Text = "\r\nĐỊNH LÍ.\t";
								rangeend.Font.Bold = 1;
								rangeend.Font.Size = 12;
							...
							else if (rangeend.Text.Contains("hq"))
							{
								rangeend.Text = "\r\nHỆ QUẢ.\t";
								rangeend.Font.Bold = 1;
								rangeend.Font.Size = 12;
							...
							else if (rangeend.Text.Contains("dn"))
							{
								rangeend.Text = "\r\nĐỊNH NGHĨA.\t";
								rangeend.Font.Bold = 1;
								rangeend.Font.Size = 12;
								rangeend.Font.Color = WdColor.wdColorGold;
								find.Wrap = WdFindWrap.wdFindContinue;
							}
							else if (rangeend.Text.Contains("nx"))
							{
								rangeend.Text = "\r\nNHẬN XÉT.\t";
								rangeend.Font.Bold = 1;
								rangeend.Font.Size = 12;
							...
							else if (rangeend.Text.Contains("cy"))
							{
								rangeend.Text = "\r\nChú ý";
								rangeend.Font.Bold = 1;
								rangeend.Font.Size = 12;
				...
				find.Execute("&", false, false, false, false, false, missing, missing, missing, "", WdReplace.wdReplaceAll);
				find = rangeend.Find;
				find.Execute(@"\lq\lq", false, false, false, false, false, missing, missing, missing, ":'", WdReplace.wdReplaceAll);
				find = rangeend.Find;
				find.Execute(@"\rq\rq", false, false, false, false, false, missing, missing, missing, "'", WdReplace.wdReplaceAll);
				find = rangeend.Find;
				find.Text = @"[a-z]{1}\).  ";
				find.MatchWildcards = true;
				while (find.Execute())
