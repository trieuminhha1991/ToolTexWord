...
namespace QuanLyTex
{
	...
	/// </summary>
	public partial class UserControl3D : System.Windows.Controls.UserControl
	{
		...
		private void MaterialButton_Click(object sender, RoutedEventArgs e)
		{
			try
			{
				...
				srt6 = @"[0-9]";
				string type = "ex";
				List<dynamic> mapTex = new List<dynamic>();
				if(All.IsChecked==true)
				{
					...
					{
						string path = appPath + @"\NganHangTex\Id5ex";
						str = str1 + str2 + str3 + str4 + str5;
						Regex rx = new Regex(str);
						mapTex = classMap.FilterId2(path, type, rx);
					}
					if (selectId6.IsChecked == true)
					{
						string path = appPath + @"\NganHangTex\Id6ex";
						str = str1 + str2 + str3 + str4 + str5 + "-" + srt6;
						Regex rx = new Regex(str);
						mapTex = classMap.FilterId2(path, type, rx);
					}
					listEcersiceY = mapTex[0];
					listEcersiceB = mapTex[1];
					listEcersiceK = mapTex[2];
					listEcersiceG = mapTex[3];
					listEcersiceT = mapTex[4];
				}
				if (SelectExer.IsChecked == true)
				{
					if (selectId5.IsChecked == true)
					{
						string path = appPath + @"\NganHangTex\Id5ex";
